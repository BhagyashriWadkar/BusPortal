package com.zensar.bus.passBooking;
...
import com.zensar.bus.exception.PassTypeException;
...
{
	if(passDetails.getPassId()==null||passDetails.getPassId()==0||passDetails.getStaffId()==null||passDetails.getRouteId()==null||passDetails.getPointId()==null||passDetails.getPassType()==null) throw new NotNullException();
	...
	int cost=0;
	if(!(passType.equalsIgnoreCase("Daily")||!(passType.equalsIgnoreCase("Weekly"))||!(passType.equalsIgnoreCase("Monthly"))))throw new PassTypeException();
	if(passType.equalsIgnoreCase("Daily"))
	{
		cost =pickupPointDAO.findCostDailyById(pointId);
	}
	else if(passType.equalsIgnoreCase("Weekly"))
	{
		cost =pickupPointDAO.findCostWeeklyById(pointId);
	}
	else if(passType.equalsIgnoreCase("Monthly"))
	{
